...
class CreateEcosystemServiceValueRasterAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ...
        """
        return self.tr("This algorithm takes an Input NLCD raster (should be the clipped raster from Step 1) and an Input ESV table (should be the Output ESV table from Step 1) and creates a new raster by mapping each NLCD value from the input raster to a corresponding per-pixel ecosystem service value (of the user’s choice) from the input table.\n Input NLCD raster: This should be the output clipped raster from Step 1, an NLCD layer clipped by a region of interest. Just like the Input NLCD raster of Step 1, this raster should be in EPSG:102003, have 30x30 meter pixels, and have NLCD values as its pixel values.\n Input ESV table: This should be the output ESV table from step 1 and should not be altered.\n The next two fields, “Input ecosystem service to create raster for” and “statistic to create ESV raster for”, specify which ecosystem service you want to map and whether you want to map minimum, average, or maximum values from the ESV table. See “Help” for more information on value origins and ecosystem service descriptions.")
